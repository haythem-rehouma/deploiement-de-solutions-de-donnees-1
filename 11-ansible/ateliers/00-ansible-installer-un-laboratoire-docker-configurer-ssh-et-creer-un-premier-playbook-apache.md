...
| Fonction | `>>` (Append) | `tee` |
|----------|---------------|-------|
| Écrire dans un fichier uniquement | Oui | Non (écrit et affiche) |
| Ajouter au fichier (append) | Oui | Oui (avec `-a`) |
| Afficher dans le terminal | Non | Oui |
| Peut nécessiter `sudo` pour fichiers protégés | Non (direct `sudo`) | Oui (ex: `echo ... | sudo tee`) |
